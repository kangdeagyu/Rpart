...
class ChartGetX extends GetxController {
  ...
  Future<void> getChartJSON(String address, String floor) async {
    var url = Uri.parse(
        'http://localhost:8080/getPrice?address=$address&floor=$floor');
    ...
    if (results == "EMPTY") {
      // api에서 조회된 데이터가 없는 경우 쓰레기값 보여준다.
      priceDataList.add(PriceData("202108", 10000));
      priceDataList.add(PriceData("202307", 10000));
    } else {
      print(results);
      for (var resultObject in results) {
        // 월별 데이터 24번
        for (var entry in resultObject.entries) {
          // 한 월에 여러 데이터를 들고오기때문
